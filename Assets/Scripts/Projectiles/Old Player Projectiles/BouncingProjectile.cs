...
public class BouncingProjectile : MonoBehaviour, IProjectile
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag != parentTag && !other.TryGetComponent(out IProjectile iProj) && !other.isTrigger) // Target is not friendly
        {
            ...
            //Else if stops the projectile from bouncing if the entity is a character
            else if(numBounces > 0)
            {
                numBounces -= 1;
                if(Physics.Raycast(transform.position, transform.forward, out var hitInfo, Mathf.Infinity, LayerMask.GetMask("Untraversable", "Hurtbox", "SeeThrough"), QueryTriggerInteraction.Ignore));
                {
                    transform.LookAt(transform.position + Vector3.Reflect(transform.forward, hitInfo.normal));
                    return;
                }
            }
            if (impactVFX != null)
