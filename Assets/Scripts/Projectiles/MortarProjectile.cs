...
public class MortarProjectile : Projectile
{
    ...
    protected Transform trackedEnemy;

    public Projectile InitMortar(GameObject a_explosionEffect, float a_radius, Transform a_trackedEnemy)
    {
        explosionEffect = a_explosionEffect;
        radius = a_radius;
        trackedEnemy = a_trackedEnemy;
    ...
    protected override void ProjectileUpdate()
    {
        ...
        {
            transform.position = new Vector3(trackedEnemy.position.x, transform.position.y, trackedEnemy.position.z);
        }
    ...
    protected override void OnTriggerEnter(Collider other) // Called when something enters the projectiles trigger zone
    {
        ...
        if (forceCollide || (other.gameObject != parentObject && !other.gameObject.CompareTag(parentTag) && !other.TryGetComponent(out Projectile _proj) && !other.isTrigger))
        {
