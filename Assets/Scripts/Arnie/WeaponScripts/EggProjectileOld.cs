...
public class EggProjectileOld : MonoBehaviour, IProjectile
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag != parentTag && !other.TryGetComponent(out IProjectile iProj) && !other.isTrigger) // Target is not friendly
        {
            ...
            if(rand == 1)
            {
                GameObject instantiatedItem = Instantiate(Chick);
                instantiatedItem.transform.position = gameObject.transform.position;
            }
