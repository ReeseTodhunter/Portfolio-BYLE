...
public class BTMajesticusShape : BTNode
{
    ...
    //Constructor
    public BTMajesticusShape(BTAgent _agent, GameObject projectile, Projectiles _byleProjectile, ProjectileMats _mat, float _speed, float _damage, attackShape _shape, float _windupDuration, float _projectileCount, string _animName = "")
    {
        agent = _agent;
    ...
    public override NodeState Evaluate(BTAgent agent) //Evaluate node output
    {
        ...
        switch (shape) //Get shape positions
        {
            case attackShape.circle:
                projectilePositions = GetProjectilePositions(((int)projectileCount), 5, ((int)projectileCount));
                break;
    ...
    private void FireShape() //Fire all projectile at player
    {
        ...
        foreach (GameObject currentProj in projectiles)
        {
            ...
            ////
            projectiles.Remove(currentProj);
            interval = 0.2f;
            return;
        }
