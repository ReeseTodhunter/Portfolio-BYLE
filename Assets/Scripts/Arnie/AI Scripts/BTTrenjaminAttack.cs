...
public class BTTrenjaminAttack : BTNode
{
    /*
    ...
    //constructor
    public BTTrenjaminAttack(BTAgent _agent, GameObject _prefab, int _projectilesPerLayer, int _layers, float _spreadInDegrees, float _speed, float _damage, string _animName = "", float _delay = 0, float _timeBetweenLayers = 0.33f, float _cooldown = 5)
    {
        agent = _agent;
    ...
    public override void UpdateNode(BTAgent agent) //update node
    {
        ...
        float angleInterval = arc / projectilesPerLayer;
        ...
        for (int i = 0; i < projectilesPerLayer; i++)
        {
            ...
            GameObject projectile = GameObject.Instantiate(bulletPrefab, spawnPos, ProjectileQuat); //instantiate projectile
            BasicProjectile projectileScript = projectile.GetComponent<BasicProjectile>(); //init projectile
            projectileScript.Init(speed, 0, 5, damage, "Enemy");
        }
        currLayer++;
