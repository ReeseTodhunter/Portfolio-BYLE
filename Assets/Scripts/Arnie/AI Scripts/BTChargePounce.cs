...
public class BTChargePounce : BTNode
{
    ...
    public BTChargePounce(float _maxPounceDistance, float _windUpDuration, float _pounceSpeed, float _overshootDistance = 10, float _damage = 25, float _hitRange = 1.5f, bool _leaveTrail = false, bool _buildUpParticles = false)
    {
    ...
    public override NodeState Evaluate(BTAgent _agent)
    {
        ...
        if (nearestNode.GetLineOfSight() && nearestNode.GetDistance() < maxPounceDistance)
        {
            ...
            if (leaveTrail)
            {
                ...
                if (trail == null) Debug.Log("go fuck yourself");
        ...
        switch (currentState)
        {
            ...
            case actionState.calculateTarget:
                if (tryGetEndPos())
                {
                    ...
                    if (trail != null) { trail.GetComponent<ParticleSystem>().Play(); }
                    break;
                }
                else
                {
