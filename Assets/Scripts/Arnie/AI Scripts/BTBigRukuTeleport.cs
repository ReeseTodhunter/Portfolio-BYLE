...
public class BTBigRukuTeleport : BTNode
{
    /*
    ...
    //Constructor
    public BTBigRukuTeleport(BTAgent _agent, float _teleportCooldown, float _windUpDuration, float _maxTeleportDistance, bool _lineOfSight, float _minDistanceFromPlayer, bool _usesVFX = false, string _animName = "", string _vfxPath = "", float _yPos = .75f, bool _loopingVFX = true)
    {
        agent = _agent;
    ...
    public override void UpdateNode(BTAgent agent) //update node
    {
        ...
        if (timer > teleportWindupDuration)
        {
            ...
            float angleInterval = 30 / 3;
            for (int i = 0; i < 3; i++) //Create all the projectiles in an arc
            {
                ...
                spawnPos.y = 1f;
                GameObject projectile = GameObject.Instantiate(Resources.Load("Projectiles/ElliptoidProjectile") as GameObject, spawnPos, ProjectileQuat); //instantiate projectile
                projectile.GetComponent<Projectile>().Init(20, 0, 6, 15, agent.gameObject); //init projectile
