...
public class BTBigRukuBlast : BTNode
{
    /*
    ...
    //constructor
    public BTBigRukuBlast(BTAgent _agent, string _prefab, int _projectilesPerLayer, int _layers, float _speed, float _damage, string _animName = "", float _delay = 0, float _timeBetweenLayers = 0.33f, float _cooldown = 5)
    {
        agent = _agent;
        bulletPrefab = Resources.Load(_prefab) as GameObject;
    ...
    public override void UpdateNode(BTAgent agent) //update node
    {
        ...
        {
            Vector3 projectileRot = eulerRot + (i * Vector3.up * angleInterval ) + new Vector3(0,currLayer * 10, 0);
            projectileRot.x = 0;
            projectileRot.z = 0;
